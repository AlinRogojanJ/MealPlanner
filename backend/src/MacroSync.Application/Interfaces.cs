...
public interface IMealPlanService
{
    ...
    /// <summary>Create a plan for a week; returns the existing one if already there.
    /// With copyFrom set, meals from that week are copied over (re-solved against current targets).</summary>
    Task<WeekPlanDto?> CreateWeekPlanAsync(Guid householdId, DateOnly weekStart, DateOnly? copyFrom = null, CancellationToken ct = default);

    /// <summary>Remove a dish from the calendar.</summary>
    Task<bool> DeleteMealAsync(Guid plannedMealId, CancellationToken ct = default);
